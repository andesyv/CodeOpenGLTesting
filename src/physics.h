...
template <typename T>
void calcPhysics(entt::registry& EM, T entities, float deltaTime = 0.f)
{
    if (deltaTime <= MIN_TICK_TIME)
        return;

    const auto time = static_cast<double>(deltaTime);
    std::vector<std::pair<entt::entity, entt::entity>> collidedObjects{entities.size()};
    ...
    for (auto it{entities.begin()}; it != entities.end(); ++it, ++i)
    {
        ...
        for (auto other{entities.begin()}; other != entities.end(); ++other)
        {
            ...
            if (isColliding(t, t2)) {
                collidedObjects.emplace_back(entt::entity{*it}, entt:entity{*other});
            }
        }
    ...
    for (auto it{collidedObjects.begin()}; it != collidedObjects.end(); ++it) {
        ...
        for (auto prev{collidedObjects.begin()}; prev != it; ++prev) {
            if (it->first == prev->second && it->second == prev->first) {
        ...
        std::cout << "Entity1 still valid: " << EM.valid(it->first) << ", Entity2 still valid: " << EM.valid(it->second) << std::endl;

        auto& [t1, p1] = entities.get<component::trans, component::phys>(it->first);
        auto& [t2, p2] = entities.get<component::trans, component::phys>(it->second);
